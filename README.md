...
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=M-RIZWAN-MD&fontSize=72&fontColor=fff&animation=twinkling&fontAlignY=32&desc=High%20Performance%20WhatsApp%20Bot&descAlignY=55&descSize=20" width="100%"/>
...
[![Version](https://img.shields.io/badge/Version-6.0.0-blue?style=for-the-badge&logo=github)](https://github.com/m-rizwan-mods/Rizwan-mdx-bot)
...
[![Stars](https://img.shields.io/github/stars/m-rizwan-mods/Rizwan-mdx-bot?style=for-the-badge&logo=starship&color=gold)](https://github.com/m-rizwan-mods/Rizwan-mdx-bot/stargazers)
[![Forks](https://img.shields.io/github/forks/m-rizwan-mods/Rizwan-mdx-bot?style=for-the-badge&logo=git&color=orange)](https://github.com/m-rizwan-mods/Rizwan-mdx-bot/network/members)
...
```bash
git clone https://github.com/m-rizwan-mods/Rizwan-mdx-bot.git
cd Rizwan-mdx-bot
npm install
cp sample.env .env
...
```env
SESSION_ID=m-rizwan-mods/Rizwan-mdx-bot_xxxxxxxxxxxxxxxxxxxxxxxx
```
...
```env
# ── REQUIRED (choose one) ────────────────────────────────────
SESSION_ID=m-rizwan-mods/Rizwan-mdx-bot_your_gist_id_here
# OR
PAIRING_NUMBER=923001234567
...
# ── BOT IDENTITY ─────────────────────────────────────────────
BOT_NAME=M-RIZWAN-MD-PRO
BOT_OWNER=M RIZWAN
PACKNAME=M-RIZWAN-MD
...
```bash
sudo bash <(curl -fsSL https://raw.githubusercontent.com/m-rizwan-mods/Rizwan-mdx-bot/main/lib/install.sh)
```
> [!IMPORTANT]
...
```bash
# After install:
nano /root/Rizwan-mdx-bot/.env
cd /root/Rizwan-mdx-bot && pm2 start dist/index.js --name m-rizwan-md
...
tmux new -s m-rizwan-md    # Start new session
npm start

# Detach:     Ctrl+B → D
# Re-attach:  tmux attach -t m-rizwan-md
# List:       tmux ls
# Kill:       tmux kill-session -t m-rizwan-md
```
...
# Keep alive with PM2
npm install -g pm2
pm2 start dist/index.js --name m-rizwan-md
pm2 save && pm2 startup
```
...
| Command | Description |
|---|---|
| `pm2 logs m-rizwan-md` | Live logs |
| `pm2 restart m-rizwan-md` | Restart |
| `pm2 stop m-rizwan-md` | Stop |
...
2. Paste: `https://github.com/m-rizwan-mods/Rizwan-mdx-bot`
3. Open **Secrets** tab (🔒) and add:

   | Key | Value |
   |---|---|
   | `SESSION_ID` | `m-rizwan-mods/Rizwan-mdx-bot_your_gist_id` |
...
**One-line Deployer:**
```bash
bash <(curl -s https://raw.githubusercontent.com/m-rizwan-mods/Rizwan-mdx-bot/main/lib/heroku.sh)
```
**Manual:**
```bash
heroku login
...
heroku config:set SESSION_ID=m-rizwan-mods/Rizwan-mdx-bot_your_gist_id
...
**One-line Deployer:**
```bash
bash <(curl -s https://raw.githubusercontent.com/m-rizwan-mods/Rizwan-mdx-bot/main/lib/fly.sh)
```
**Manual:**
```bash
...
fly secrets set SESSION_ID=m-rizwan-mods/Rizwan-mdx-bot_your_gist_id
...
```bash
# Build image
docker build -t m-rizwan-md .

# Run
docker run -d \
  -e SESSION_ID=m-rizwan-mods/Rizwan-mdx-bot_your_gist_id \
...
```env
# MongoDB
MONGO_URL=mongodb+srv://user:password@cluster.mongodb.net/mrizwanmd

# PostgreSQL
POSTGRES_URL=postgresql://user:password@host:5432/mrizwanmd

# MySQL
MYSQL_URL=mysql://user:password@host:3306/mrizwanmd
...
| `BOT_NAME` | ❌ | `M-RIZWAN-MD` | Bot display name |
| `BOT_OWNER` | ❌ | `M RIZWAN` | Owner display name |
| `PACKNAME` | ❌ | `M-RIZWAN-MD` | Sticker pack name |
...
export default {
    ...
    async handler(sock: any, message: any, args: any[], context: any = {}) {
        const {
            ...
            channelInfo       // M-RIZWAN-MD branding spread
        } = context;
...
> [!IMPORTANT]
> - Verify `SESSION_ID` starts with `m-rizwan-mods/Rizwan-mdx-bot_`
...
[![WhatsApp](https://img.shields.io/badge/WhatsApp_Channel-25D366?style=for-the-badge&logo=whatsapp&logoColor=white)](https://whatsapp.com/channel/0029Vb4Jh4wHgZWirfaYva0H)
[![GitHub Issues](https://img.shields.io/badge/GitHub_Issues-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/m-rizwan-mods/Rizwan-mdx-bot/issues)
...
[MIT License](LICENSE) · Made with ❤️ by **M RIZWAN** · [M RIZWAN](https://github.com/m-rizwan-mods)
