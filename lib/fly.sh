#!/bin/bash
# M-RIZWAN-MD Fly.io One-Click Deployer
...
echo -e "${CYAN}"
echo "╔═══════════════════════════════════════╗"
echo "║        M-RIZWAN-MD Fly.io Deployer        ║"
echo "║          by M RIZWAN            ║"
echo "╚═══════════════════════════════════════╝"
echo -e "${NC}"
...
read -p "$(echo -e ${CYAN}App name (e.g. m-rizwan-md): ${NC})" APP_NAME
APP_NAME=${APP_NAME:-m-rizwan-md-bot}

read -p "$(echo -e ${CYAN}Session ID (M RIZWAN/M-RIZWAN-MD_xxxxx): ${NC})" SESSION_ID
...
OWNER_NUMBER=${OWNER_NUMBER:-923436259742}

read -p "$(echo -e ${CYAN}Bot name (default: M-RIZWAN-MD): ${NC})" BOT_NAME
BOT_NAME=${BOT_NAME:-M-RIZWAN-MD}
...
if [ ! -f "fly.toml" ]; then
    echo -e "${YELLOW}📦 Cloning M-RIZWAN-MD repo...${NC}"
    git clone https://github.com/m-rizwan-mods/Rizwan-mdx-bot m-rizwan-md-deploy
    cd m-rizwan-md-deploy
fi
